...
func testUserToManyUsersPDFS(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	if err = randomize.Struct(seed, &b, usersPDFDBTypes, false, usersPDFColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	if err = randomize.Struct(seed, &c, usersPDFDBTypes, false, usersPDFColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	...
	check, err := a.UsersPDFS().All(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	slice := UserSlice{&a}
	if err = a.L.LoadUsersPDFS(ctx, tx, false, (*[]*User)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.UsersPDFS); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
	}

	a.R.UsersPDFS = nil
	if err = a.L.LoadUsersPDFS(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.UsersPDFS); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testUserToManyAddOpUsersPDFS(t *testing.T) {
	var err error
	...
	foreignersSplitByInsertion := [][]*UsersPDF{
		{&b, &c},
		{&d, &e},
	}

	for i, x := range foreignersSplitByInsertion {
		err = a.AddUsersPDFS(ctx, tx, i != 0, x...)
		if err != nil {
			t.Fatal(err)
		}
		...
		if a.R.UsersPDFS[i*2] != first {
			t.Error("relationship struct slice not set to correct value")
		}
		if a.R.UsersPDFS[i*2+1] != second {
			t.Error("relationship struct slice not set to correct value")
		}
...
var (
	userDBTypes = map[string]string{`ID`: `uuid`, `CreatedAt`: `timestamp without time zone`}
	_           = bytes.MinRead
)
